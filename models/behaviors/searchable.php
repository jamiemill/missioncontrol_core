...
Added sanitization to the search() method too.
...
class SearchableBehavior extends ModelBehavior {
	...
	// TODO: the html_entity_decode function here is not PHP4 compatible, it produces the following error: 
	// Warning (2): cannot yet handle MBCS in html_entity_decode()!
	...
	function updateSearchIndex(&$model, $id = null) {
		$this->model = &$model;
		if(!$id) {
			return false;
		}
		$this->_indexForId = $id;
		$this->model->data = $this->model->read(null,$id);
		$this->_index = $this->_indexData();
		$this->afterSave();
	}
